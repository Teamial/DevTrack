...
var Tracker = class extends import_events2.EventEmitter {
  ...
  constructor(outputChannel, trackingDir) {
    ...
    this.idleTimeoutMs = 15 * 60 * 1e3;
    this.trackKeystrokesEnabled = true;
    // Emit metrics while active (throttled) so adaptive scheduling can react
    this.lastMetricsEmitTs = 0;
    this.METRICS_EMIT_THROTTLE_MS = 30 * 1e3;
    // 30 seconds
  ...
  async initializeWatcher() {
    try {
      ...
      this.trackKeystrokesEnabled = config.get("trackKeystrokes", true);
      const maxIdleSeconds = config.get("maxIdleTimeBeforePause", 900);
      const clamped = Math.max(60, Math.min(24 * 60 * 60, maxIdleSeconds));
      this.idleTimeoutMs = clamped * 1e3;
...
var Scheduler2 = class {
  ...
  handleActivityMetrics(metrics) {
    if (this.options.enableAdaptiveScheduling && this.isRunning && !this.isCommitting) {
      ...
      if (timeSinceLastCommit > this.commitFrequency * 60 * this.options.adaptiveEarlyCommitAfterFraction && distinctFiles >= this.options.adaptiveMinDistinctFiles && metrics.activeTime > this.options.minActiveTimeForCommit) {
        if (metrics.keystrokes > 0 && metrics.keystrokes < this.options.adaptiveMinKeystrokes) {
        } else {
          this.outputChannel.appendLine(
...
async function handleConfigurationChange(services) {
  try {
    ...
    if (services.scheduler) {
      ...
      const maxIdleTimeBeforePause = config.get("maxIdleTimeBeforePause") || 900;
      const adaptiveEarlyCommitAfterFraction = config.get("adaptiveEarlyCommitAfterFraction") ?? 0.5;
      const adaptiveMinDistinctFiles = config.get("adaptiveMinDistinctFiles") ?? 3;
      const adaptiveMinKeystrokes = config.get("adaptiveMinKeystrokes") ?? 120;
    ...
    services.tracker.updateTrackingSettings({
      maxIdleTimeBeforePauseSeconds: config.get("maxIdleTimeBeforePause") || 900,
      trackKeystrokes: config.get("trackKeystrokes", true)
    });
