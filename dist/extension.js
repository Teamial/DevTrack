...
var GitService = class extends import_events.EventEmitter {
  constructor(outputChannel) {
    super();
    this.outputChannel = outputChannel;
    ...
    this.trackingDirectory = path2.join(this.repoPath, ".devtrack");
    this.initialCommitTimestamp = Date.now();
    if (!path2.isAbsolute(this.repoPath)) {
  ...
  async isDevTrackRepo() {
    try {
      const devtrackPath = path2.join(this.repoPath, ".devtrack");
      await vscode2.workspace.fs.stat(vscode2.Uri.file(devtrackPath));
      return true;
    } catch {
      return false;
    }
  }
  async configureRemote(remoteUrl) {
    try {
  ...
  async setupInitialConfig(remoteUrl) {
    try {
  ...
  async setupGitignore() {
    const gitignorePath = path2.join(this.repoPath, ".gitignore");
    ...
    await vscode2.workspace.fs.writeFile(
      vscode2.Uri.file(gitignorePath),
      Buffer.from(defaultIgnores, "utf8")
    );
  }
  async setupDevTrackDirectory() {
    await vscode2.workspace.fs.createDirectory(
      vscode2.Uri.file(this.trackingDirectory)
    );
    const readmePath = path2.join(this.trackingDirectory, "README.md");
...
## Configuration
- Commit Frequency: ${vscode2.workspace.getConfiguration("devtrack").get("commitFrequency")} minutes
- Excluded Patterns: ${JSON.stringify(vscode2.workspace.getConfiguration("devtrack").get("exclude"))}
`;
    await vscode2.workspace.fs.writeFile(
      vscode2.Uri.file(readmePath),
  ...
  async getModifiedFiles() {
    try {
      ...
      const config = vscode2.workspace.getConfiguration("devtrack");
      ...
      return status.files.filter((file) => {
        const isExcluded = excludePatterns.some(
          (pattern) => minimatch(file.path, pattern, { dot: true })
        );
        const isDevTrackFile = file.path.startsWith(".devtrack/") || file.path === ".gitignore";
        const fullPath = path2.join(this.repoPath, file.path);
        ...
        return !isExcluded && !isDevTrackFile && isInWorkspace && (isDeleted || fileExists);
      }).map((file) => file.path);
    } catch (error) {
      this.outputChannel.appendLine(
...
var Scheduler2 = class {
  ...
  async commitChanges() {
    ...
    const config = import_vscode.workspace.getConfiguration("devtrack");
    const confirmBeforeCommit = config.get(
      "confirmBeforeCommit",
      true
    );
    if (confirmBeforeCommit) {
      const userResponse = await import_vscode.window.showInformationMessage(
...
async function activate(context) {
  const outputChannel = vscode6.window.createOutputChannel("DevTrack");
  ...
  const excludePatterns = config.get("exclude") || [];
  const confirmBeforeCommit = config.get("confirmBeforeCommit") || true;
  ...
  const auth2 = vscode6.authentication;
  let session;
  const trackingStatusBar = vscode6.window.createStatusBarItem(
  ...
  const authStatusBar = vscode6.window.createStatusBarItem(
    vscode6.StatusBarAlignment.Right,
    101
  );
  ...
  async function handleLogout() {
    ...
    const loginChoice = await vscode6.window.showInformationMessage(
      "DevTrack: Successfully logged out. Would you like to log in with a different account?",
      "Yes",
      "No"
    );
    if (loginChoice === "Yes") {
      vscode6.commands.executeCommand("devtrack.login");
    }
    outputChannel.appendLine("DevTrack: User logged out.");
  }
  try {
    session = await auth2.getSession("github", ["repo", "read:user"], {
      createIfNone: false
    });
    if (session) {
      ...
      authStatusBar.text = "$(mark-github) DevTrack: Not Connected";
      authStatusBar.tooltip = "DevTrack Status";
      const setupChoice = await vscode6.window.showInformationMessage(
    ...
    const startTracking = vscode6.commands.registerCommand(
      ...
      () => {
        if (scheduler) {
          ...
          trackingStatusBar.text = "$(clock) DevTrack: Tracking";
          trackingStatusBar.tooltip = "DevTrack: Tracking your coding activity is active";
          vscode6.window.showInformationMessage("DevTrack: Tracking started.");
    ...
    const stopTracking = vscode6.commands.registerCommand(
      ...
      () => {
        if (scheduler) {
          ...
          trackingStatusBar.text = "$(circle-slash) DevTrack: Stopped";
          trackingStatusBar.tooltip = "DevTrack: Tracking is stopped";
          vscode6.window.showInformationMessage("DevTrack: Tracking stopped.");
    ...
    const loginCommand = vscode6.commands.registerCommand(
      "devtrack.login",
      async () => {
        try {
          githubService.setToken("");
          ...
          vscode6.window.showErrorMessage("DevTrack: GitHub connection failed.");
        }
      }
    );
    const logoutCommand = vscode6.commands.registerCommand(
      "devtrack.logout",
      async () => {
        await handleLogout();
      }
    ...
    vscode6.workspace.onDidChangeConfiguration(async (event) => {
      if (event.affectsConfiguration("devtrack")) {
        const newConfig = vscode6.workspace.getConfiguration("devtrack");
        const newRepoName = newConfig.get("repoName") || "code-tracking";
        const newCommitFrequency = newConfig.get("commitFrequency") || 30;
        const newExcludePatterns = newConfig.get("exclude") || [];
        const newConfirmBeforeCommit = newConfig.get("confirmBeforeCommit") || true;
    ...
    vscode6.window.showErrorMessage("DevTrack: GitHub authentication failed.");
  }
}
function deactivate() {
}
