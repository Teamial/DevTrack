...
function initTask(bare = false, path5, customArgs) {
  ...
  return {
    ...
    format: "utf-8",
    parser(text) {
      return parseInit(commands2.includes("--bare"), path5, text);
    }
  };
}
var bareCommand;
...
var _GitService = class _GitService extends import_events.EventEmitter {
      ...
      try {
        ...
        if (homeDir) {
          ...
          try {
            ...
            await fs.promises.writeFile(gitConfig, "", { mode: 420 });
          }
        }
      } catch (error) {
        this.outputChannel.appendLine(
  ...
  async verifyGitConfig() {
    try {
      ...
      try {
        ...
        (0, import_child_process3.execSync)(`${versionCmd} --version`, { encoding: "utf8" });
        this.outputChannel.appendLine(
    ...
    for (let attempt = 1; attempt <= _GitService.MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt < _GitService.MAX_RETRIES) {
          await new Promise(
            (resolve) => setTimeout(resolve, _GitService.RETRY_DELAY * attempt)
          );
    ...
    return this.enqueueOperation(async () => {
      try {
        ...
        await this.withRetry(async () => {
          ...
          if (status.files.length > 0 || !status.staged.length || status.not_added.length > 0) {
            await this.git.add(".");
        ...
        await this.withRetry(async () => {
          try {
            ...
            } else if (error.message.includes("src refspec main does not match any")) {
              await this.git.checkout(["-B", "main"]);
  ...
  handleGitError(error) {
    ...
    if ((_a2 = error.message) == null ? void 0 : _a2.includes("ENOENT")) {
      errorMessage = process.platform === "win32" ? "Git not found. Please install Git for Windows from https://git-scm.com/download/win" : "Git is not accessible. Please ensure Git is installed.";
    } else if ((_b = error.message) == null ? void 0 : _b.includes("spawn git ENOENT")) {
      errorMessage = process.platform === "win32" ? "Git not found in PATH. Please restart VS Code after installing Git." : "Failed to spawn Git process. Please verify your Git installation.";
...
var Tracker = class extends import_events2.EventEmitter {
  ...
  handleChange(uri, type) {
    ...
    try {
      ...
      if (!isExcluded) {
        const fileExt = (_a2 = uri.fsPath.split(".").pop()) == null ? void 0 : _a2.toLowerCase();
...
var ProjectContext = class {
  ...
  getContextForSummary(currentChanges) {
    let context = "";
    try {
      const currentFiles = currentChanges.map((change) => path3.basename(change.uri.fsPath)).filter((file, index, self) => self.indexOf(file) === index);
...
var SummaryGenerator = class {
  ...
  async getFileChanges(change) {
    try {
      ...
      return this.parseDiff(diff, path4.basename(change.uri.fsPath));
    } catch (error) {
      this.outputChannel.appendLine(`Error getting file changes: ${error}`);
      return "";
    }
  }
  parseDiff(diff, filename) {
    if (!diff) {
      return filename;
    }
...
"${commitMessage}"`,
      ...
      if (this.pendingChanges) {
        ...
        setTimeout(() => this.commitChanges(), 5e3);
      }
    }
  }
...
var GitInstallationHandler = class {
  static async checkGitInstallation(outputChannel) {
    try {
      ...
      const response = await vscode7.window.showErrorMessage(
...
- Generate detailed summaries of your work
  ...
  GitInstallationHandler.checkGitInstallation(outputChannel).then(
    (gitInstalled) => {
      ...
      vscode7.window.showInformationMessage(message, "Get Started", "Learn More", "Later").then((selection) => {
        if (selection === "Get Started") {
          vscode7.commands.executeCommand("devtrack.login");
        } else if (selection === "Learn More") {
          vscode7.window.showInformationMessage(welcomeMessage, "Set Up Now", "Later").then((choice) => {
...
async function activate(context) {
  let testCommand = vscode7.commands.registerCommand("devtrack.test", () => {
...
async function restoreAuthenticationState(context, services) {
  var _a2, _b;
  try {
    const session = await vscode7.authentication.getSession(
      "github",
      ["repo", "read:user"],
      {
        createIfNone: false,
        silent: true
        // Try to get session without prompting user
      }
    );
    if (session) {
      ...
      if (username) {
        ...
        const currentWorkspace = (_b = (_a2 = vscode7.workspace.workspaceFolders) == null ? void 0 : _a2[0]) == null ? void 0 : _b.uri.fsPath;
        const config = vscode7.workspace.getConfiguration("devtrack");
        const repoName = config.get("repoName") || (persistedState == null ? void 0 : persistedState.repoName) || "code-tracking";
...
async function initializeServices(context) {
  var _a2;
  const outputChannel = vscode7.window.createOutputChannel("DevTrack");
  context.subscriptions.push(outputChannel);
  outputChannel.appendLine("DevTrack: Extension activated.");
  if (!((_a2 = vscode7.workspace.workspaceFolders) == null ? void 0 : _a2.length)) {
  ...
  if (!authRestored) {
    ...
    if (shouldPrompt) {
      const response = await vscode7.window.showInformationMessage(
...
function createStatusBarItem(type) {
  const item = vscode7.window.createStatusBarItem(
...
async function loadConfiguration(services) {
  const config = vscode7.workspace.getConfiguration("devtrack");
...
async function registerCommands(context, services) {
  const startTracking = vscode7.commands.registerCommand(
    ...
    async () => {
      ...
      try {
        ...
        if (services.scheduler) {
          ...
          updateStatusBar(services, "tracking", true);
          vscode7.window.showInformationMessage("DevTrack: Tracking started.");
        } else {
          const response = await vscode7.window.showInformationMessage(
  ...
  const stopTracking = vscode7.commands.registerCommand(
    "devtrack.stopTracking",
    () => {
      if (services.scheduler) {
        services.scheduler.stop();
        updateStatusBar(services, "tracking", false);
        vscode7.window.showInformationMessage("DevTrack: Tracking stopped.");
        services.outputChannel.appendLine(
  ...
  const loginCommand = vscode7.commands.registerCommand(
    "devtrack.login",
    async () => {
      var _a2;
      try {
  ...
  const logoutCommand = vscode7.commands.registerCommand(
    "devtrack.logout",
  ...
  try {
    ...
    if (!await GitInstallationHandler.checkGitInstallation(
      ...
      if (response === "Show Guide") {
        await vscode7.commands.executeCommand("devtrack.showGitGuide");
      }
      throw new Error(
    ...
    try {
      ...
      await services.extensionContext.globalState.update("devtrackAuthState", {
        username,
        repoName,
        lastWorkspace: (_b = (_a2 = vscode7.workspace.workspaceFolders) == null ? void 0 : _a2[0]) == null ? void 0 : _b.uri.fsPath
      });
      updateStatusBar(services, "auth", true);
      updateStatusBar(services, "tracking", true);
      services.outputChannel.appendLine(
...
async function setupRepository(services, repoName, remoteUrl) {
...
async function handleLogout(services) {
  ...
  cleanup(services);
  const loginChoice = await vscode7.window.showInformationMessage(
...
function handleError(services, context, error) {
  ...
  vscode7.window.showErrorMessage(`DevTrack: ${message}`);
}
async function ensureGitHubAuth(services) {
  var _a2;
  try {
    try {
      const session2 = await vscode7.authentication.getSession(
        "github",
        ["repo", "read:user"],
        {
          createIfNone: false,
