...
var GitHubService = class {
   ...
   */
  async createRepo(repoName, description = "DevTrack Repository") {
    try {
...
var _GitService = class _GitService extends EventEmitter2 {
  constructor(outputChannel) {
    super();
    ...
    this.isWindows = process3.platform === "win32";
    ...
    const homeDir = process3.env.HOME || process3.env.USERPROFILE || "";
  ...
  async createTrackingDirectory() {
    try {
      if (!this.currentTrackingDir) {
        ...
        const workspaceFolders = vscode2.workspace.workspaceFolders;
        if (workspaceFolders && workspaceFolders.length > 0) {
          const workspaceId = Buffer2.from(workspaceFolders[0].uri.fsPath).toString("base64").replace(/[/+=]/g, "_");
          this.currentTrackingDir = path.join(
            homeDir,
            ".devtrack",
            "tracking",
  ...
  async initializeTracking() {
    try {
      ...
      this.projectIdentifier = Buffer2.from(projectPath).toString("base64").replace(/[/+=]/g, "_");
      this.currentTrackingDir = path.join(
  ...
  findGitExecutable() {
    try {
      if (this.isWindows) {
        const pathEnv = process3.env.PATH || "";
        ...
        try {
          const gitPathFromWhere = execSync("where git", { encoding: "utf8" }).split("\n")[0].trim().replace(/\\/g, "/");
  ...
  async verifyGitConfig() {
    try {
      ...
      const normalizedGitPath = this.isWindows ? gitPath.replace(/\\/g, "/") : gitPath;
      try {
        const versionCmd = this.isWindows ? `"${normalizedGitPath}"` : normalizedGitPath;
  ...
  async withRetry(operation) {
    let lastError = null;
    for (let attempt = 1; attempt <= _GitService.MAX_RETRIES; attempt++) {
      try {
        return await operation();
      } catch (error) {
        lastError = error;
        this.outputChannel.appendLine(
          `DevTrack: Operation failed (attempt ${attempt}/${_GitService.MAX_RETRIES}): ${error.message}`
        );
        if (attempt < _GitService.MAX_RETRIES) {
          await new Promise(
  ...
  handleGitError(error) {
    ...
    if ((_a2 = error.message) == null ? void 0 : _a2.includes("ENOENT")) {
      errorMessage = process3.platform === "win32" ? "Git not found. Please install Git for Windows from https://git-scm.com/download/win" : "Git is not accessible. Please ensure Git is installed.";
    } else if ((_b = error.message) == null ? void 0 : _b.includes("spawn git ENOENT")) {
      errorMessage = process3.platform === "win32" ? "Git not found in PATH. Please restart VS Code after installing Git." : "Failed to spawn Git process. Please verify your Git installation.";
    } else if ((_c = error.message) == null ? void 0 : _c.includes("not a git repository")) {
      errorMessage = "Not a Git repository. Please initialize the repository first.";
    }
    this.outputChannel.appendLine(
      `DevTrack: ${errorMessage} - ${error.message}`
    );
    vscode2.window.showErrorMessage(`DevTrack: ${errorMessage}`);
  }
...
var Tracker = class extends EventEmitter3 {
  ...
  async initialize() {
    var _a2;
    try {
      if (!((_a2 = vscode3.workspace.workspaceFolders) == null ? void 0 : _a2.length)) {
        this.outputChannel.appendLine(
          "DevTrack: Waiting for workspace to load..."
        );
        const disposable = vscode3.workspace.onDidChangeWorkspaceFolders(() => {
          var _a3;
          if ((_a3 = vscode3.workspace.workspaceFolders) == null ? void 0 : _a3.length) {
  ...
  async initializeWatcher() {
    try {
      const config = vscode3.workspace.getConfiguration("devtrack");
      ...
      const filePattern = new vscode3.RelativePattern(
        workspaceFolder,
      ...
      this.watcher = vscode3.workspace.createFileSystemWatcher(
        filePattern,
        false,
  ...
  shouldTrackFile(filePath) {
    ...
    const relativePath = vscode3.workspace.asRelativePath(filePath);
    ...
    const fileExt = path3.extname(filePath).toLowerCase().slice(1);
  ...
  handleChange(uri, type) {
    try {
      ...
      this.outputChannel.appendLine(
        `DevTrack: Successfully tracked ${type} in ${vscode3.workspace.asRelativePath(uri)}`
      );
      this.outputChannel.appendLine(
...
var ProjectContext = class {
          ...
          this.commitHistory = commits.map((commit) => {
            ...
            return {
              ...
              files: ((_a2 = commit.files) == null ? void 0 : _a2.map(
                (f) => vscode4.workspace.asRelativePath(f.uri)
              )) || [],
              hash: commit.hash
            };
          });
  ...
  async addCommit(summary, changes) {
    try {
      const trackedFiles = changes.map((change) => vscode4.workspace.asRelativePath(change.uri)).filter((filePath) => this.shouldTrackFile(filePath));
      const commit = {
        timestamp: Date.now(),
        summary,
        files: trackedFiles
  ...
  getContextForSummary(currentChanges) {
    let context = "";
    try {
      const currentFiles = currentChanges.map((change) => path4.basename(change.uri.fsPath)).filter((file, index, self) => self.indexOf(file) === index);
...
var SummaryGenerator = class {
  ...
  async getFileChanges(change) {
    try {
      const oldUri = change.type === "added" ? void 0 : change.uri;
      const newUri = change.type === "deleted" ? void 0 : change.uri;
      if (!oldUri && !newUri) {
      ...
      return {
        ...
        snippet: this.formatCodeSnippet(
          currentContent,
          path5.basename(change.uri.fsPath)
        )
      };
    } catch (error) {
  ...
  formatCodeSnippet(content, filename) {
...
\`\`\``;
  ...
  parseDiff(diff, uri) {
    if (!diff) {
      return path5.basename(uri.fsPath);
    }
    ...
    const descriptions = [];
    const filename = path5.basename(uri.fsPath);
...
import { setTimeout as setTimeout2, clearInterval, setInterval } from "timers";
var Scheduler2 = class {
  ...
  async commitChanges() {
    ...
    try {
      ...
      const config = vscode6.workspace.getConfiguration("devtrack");
      if (config.get("confirmBeforeCommit", true)) {
        const condensedMessage = this.createCondensedCommitMessage(commitMessage);
        const userResponse = await vscode6.window.showInformationMessage(
...
"${condensedMessage}"`,
      ...
      if (this.pendingChanges) {
        ...
        setTimeout2(() => this.commitChanges(), 5e3);
      }
    }
  }
...
var ChangeAnalyzer = class {
  ...
  async getFileContent(uri) {
    try {
      const document2 = await vscode7.workspace.openTextDocument(uri);
      return document2.getText();
    } catch (error) {
      this.outputChannel.appendLine(`Error reading file content: ${error}`);
      return "";
...
// src/extension.ts
var _GitInstallationHandler = class _GitInstallationHandler {
  static async checkGitInstallation(outputChannel) {
    try {
      const gitVersion = execSync2("git --version", { encoding: "utf8" });
      outputChannel.appendLine(`DevTrack: Git found - ${gitVersion.trim()}`);
      return true;
    } catch {
      const platform = process4.platform;
      const response = await vscode8.window.showErrorMessage(
        "Git is required but not found on your system. This might be because Git is not installed or not in your system PATH.",
        {
          modal: true,
          detail: "Would you like to view the installation guide or fix PATH issues?"
        },
        ...platform === "win32" ? ["Show Installation Guide", "Fix PATH Issue", "Cancel"] : ["Show Installation Guide", "Cancel"]
      );
      if (response === "Show Installation Guide") {
        _GitInstallationHandler.showInstallationGuide();
      } else if (response === "Fix PATH Issue") {
        _GitInstallationHandler.showPathFixGuide();
      }
      return false;
    }
  }
  ...
  static showInstallationGuide() {
    const platform = process4.platform;
    const downloadUrl = this.DOWNLOAD_URLS[platform];
...
_GitInstallationHandler.DOWNLOAD_URLS = {
  win32: "https://git-scm.com/download/win",
  darwin: "https://git-scm.com/download/mac",
  linux: "https://git-scm.com/download/linux"
};
var GitInstallationHandler = _GitInstallationHandler;
...
- Generate detailed summaries of your work
  ...
  GitInstallationHandler.checkGitInstallation(outputChannel).then(
    (gitInstalled) => {
      ...
      vscode8.window.showInformationMessage(message, "Get Started", "Learn More", "Later").then((selection) => {
        if (selection === "Get Started") {
          vscode8.commands.executeCommand("devtrack.login");
        } else if (selection === "Learn More") {
          vscode8.window.showInformationMessage(welcomeMessage, "Set Up Now", "Later").then((choice) => {
...
async function activate(context) {
  let testCommand = vscode8.commands.registerCommand("devtrack.test", () => {
...
async function restoreAuthenticationState(context, services) {
  ...
  try {
    ...
    const currentWorkspace = (_b = (_a2 = vscode8.workspace.workspaceFolders) == null ? void 0 : _a2[0]) == null ? void 0 : _b.uri.fsPath;
...
function createBaseServices(outputChannel, context) {
  const homeDir = process4.env.HOME || process4.env.USERPROFILE || "";
  const defaultTrackingDir = path6.join(
    homeDir,
    ".devtrack",
    "tracking",
    "default"
  );
  if (!fs2.existsSync(defaultTrackingDir)) {
    fs2.mkdirSync(defaultTrackingDir, { recursive: true });
  }
  return {
    outputChannel,
    githubService: new GitHubService(outputChannel),
    gitService: new GitService(outputChannel),
    tracker: new Tracker(outputChannel, defaultTrackingDir),
    // Fixed: Added trackingDir argument
...
async function initializeServices(context) {
  var _a2;
  const outputChannel = vscode8.window.createOutputChannel("DevTrack");
  ...
  const trackingBase = path6.join(homeDir, ".devtrack", "tracking");
  try {
    await fs2.promises.mkdir(trackingBase, { recursive: true });
  } catch (error) {
  ...
  if (!((_a2 = vscode8.workspace.workspaceFolders) == null ? void 0 : _a2.length)) {
    outputChannel.appendLine("DevTrack: No workspace folder open.");
    return createBaseServices(outputChannel, context);
  }
  const workspaceId = Buffer3.from(
    vscode8.workspace.workspaceFolders[0].uri.fsPath
  ).toString("base64").replace(/[/+=]/g, "_");
  const trackingDir = path6.join(trackingBase, workspaceId);
  try {
    await fs2.promises.mkdir(trackingDir, { recursive: true });
  } catch (error) {
    outputChannel.appendLine(
  ...
  if (!authRestored) {
    ...
    if (shouldPrompt) {
      const response = await vscode8.window.showInformationMessage(
...
function createStatusBarItem(type) {
  const item = vscode8.window.createStatusBarItem(
...
async function registerCommands(context, services) {
  const startTracking = vscode8.commands.registerCommand(
    ...
    async () => {
      ...
      try {
        ...
        if (services.scheduler) {
          ...
          updateStatusBar(services, "tracking", true);
          vscode8.window.showInformationMessage("DevTrack: Tracking started.");
        } else {
          const response = await vscode8.window.showInformationMessage(
  ...
  const stopTracking = vscode8.commands.registerCommand(
    "devtrack.stopTracking",
    () => {
      if (services.scheduler) {
        services.scheduler.stop();
        updateStatusBar(services, "tracking", false);
        vscode8.window.showInformationMessage("DevTrack: Tracking stopped.");
        services.outputChannel.appendLine(
  ...
  const loginCommand = vscode8.commands.registerCommand(
    "devtrack.login",
    async () => {
      var _a2;
      try {
        ...
        if ((_a2 = error.message) == null ? void 0 : _a2.includes("Cancelled")) {
          services.outputChannel.appendLine(
  ...
  const logoutCommand = vscode8.commands.registerCommand(
    "devtrack.logout",
...
async function initializeDevTrack(services) {
  ...
  try {
    ...
    await services.extensionContext.globalState.update("devtrackAuthState", {
      username,
      repoName,
      lastWorkspace: (_b = (_a2 = vscode8.workspace.workspaceFolders) == null ? void 0 : _a2[0]) == null ? void 0 : _b.uri.fsPath
    });
    services.outputChannel.appendLine(
...
async function handleLogout(services) {
  ...
  cleanup(services);
  const loginChoice = await vscode8.window.showInformationMessage(
...
function updateStatusBar(services, type, active) {
  ...
  if (type === "tracking") {
    trackingStatusBar.text = active ? "$(clock) DevTrack: Tracking" : "$(circle-slash) DevTrack: Stopped";
    trackingStatusBar.tooltip = active ? "DevTrack: Tracking your coding activity is active" : "DevTrack: Tracking is stopped";
  } else {
    authStatusBar.text = active ? "$(check) DevTrack: Connected" : "$(mark-github) DevTrack: Not Connected";
    authStatusBar.tooltip = active ? "DevTrack is connected to GitHub" : "DevTrack Status";
  }
}
function handleError(services, context, error) {
  const message = error.message || "An unknown error occurred";
  services.outputChannel.appendLine(`DevTrack: ${context} - ${message}`);
  vscode8.window.showErrorMessage(`DevTrack: ${message}`);
}
async function ensureGitHubAuth(services) {
  var _a2;
  try {
    try {
      const session2 = await vscode8.authentication.getSession(
        "github",
        ["repo", "read:user"],
        {
          createIfNone: false,
